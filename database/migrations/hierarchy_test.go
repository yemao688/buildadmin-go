...
func dropHierarchyTables(t *testing.T, db *gorm.DB, config *conf.Configuration) {
	t.Helper()
	_ = db.Migrator().DropTable(tableName(config, "admin_hierarchy_lock"))
	_ = db.Migrator().DropTable(tableName(config, "admin_closure"))
	_ = db.Migrator().DropTable(tableName(config, "admin"))
}

func countClosureRows(t *testing.T, db *gorm.DB, config *conf.Configuration) int64 {
	t.Helper()
	var count int64
	if err := db.Table(tableName(config, "admin_closure")).Count(&count).Error; err != nil {
...
func TestHierarchyVersion224CustomPrefix(t *testing.T) {
	...
	if !tableExists(db, tableName(config, "admin_closure")) {
...
func TestHierarchyVersion224LegacyUpgrade(t *testing.T) {
	...
	if err := db.Exec(
		"CREATE TABLE " + quoteIdentifier(adminTable) + " (" +
	...
	if err := db.Exec("INSERT INTO "+quoteIdentifier(adminTable)+" (id, username) VALUES (?, ?), (?, ?)", 1, "a1", 2, "a2").Error; err != nil {
...
func TestHierarchyVersion224PartialRecovery(t *testing.T) {
	...
	if err := db.Exec(
		"INSERT INTO "+quoteIdentifier(closureTable)+" (ancestor_id, descendant_id, depth) VALUES (?, ?, 0)",
		1, 1,
	).Error; err != nil {
	...
	adminTable := tableName(config, "admin")
	if err := db.Exec("DROP INDEX `idx_parent_id` ON " + quoteIdentifier(adminTable)).Error; err != nil {
...
func TestHierarchyVersion224LockTableRecovery(t *testing.T) {
	...
	if err := db.Migrator().DropTable(tableName(config, "admin_hierarchy_lock")); err != nil {
...
func TestHierarchyVersion224LockRowRecovery(t *testing.T) {
	...
	lockTable := tableName(config, "admin_hierarchy_lock")
	if err := db.Exec("DELETE FROM " + quoteIdentifier(lockTable)).Error; err != nil {
