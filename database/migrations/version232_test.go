...
func TestVersion232NormalizesOnlyInstallerRules(t *testing.T) {
	...
	recycle := tableName(cfg, "security_data_recycle")
	sensitive := tableName(cfg, "security_sensitive_data")
...
func TestVersion232LegacySensitiveUserSignatureIsRejectedAndAdoptionFailsClosed(t *testing.T) {
	...
	q := func(table string) string { return quoteIdentifier(tableName(cfg, table)) }
	...
	require.NoError(t, db.Table(tableName(cfg, "migrations")).Create(&migrationRecord{Version: 20260722000000, MigrationName: "Version232", StartTime: finished, EndTime: &finished}).Error)
	local := LocalMigrations()[9]
	count, err := AdoptCompletedLegacyAliases(db, cfg, []LocalMigration{local})
	require.Error(t, err)
	require.Zero(t, count)
	var ledgerCount int64
	require.NoError(t, db.Table(tableName(cfg, "go_migrations")).Where("sequence = ?", local.Sequence).Count(&ledgerCount).Error)
	require.Zero(t, ledgerCount)

	require.NoError(t, version232(db, cfg))
	require.NoError(t, verifySecurityRuleContract(db.Session(&gorm.Session{NewDB: true}), cfg))
	var fields string
	require.NoError(t, db.Table(tableName(cfg, "security_sensitive_data")).Where("id = 2").Pluck("data_fields", &fields).Error)
	require.NotContains(t, fields, "password")
}
