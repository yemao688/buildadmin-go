...
func TestVersion226BackfillsOwnersAndIsIdempotent(t *testing.T) {
	...
	tables := []string{core.TableName(cfg, "admin"), core.TableName(cfg, "user"), core.TableName(cfg, "user_money_log"), core.TableName(cfg, "user_score_log")}
...
func TestVersion226FreshEmptyDoesNotRequireRoot(t *testing.T) {
	...
	cfg := &conf.Configuration{Database: conf.Database{Prefix: prefix}}
	q := func(s string) string { return "`" + s + "`" }
	admin := core.TableName(cfg, "admin")
	user := core.TableName(cfg, "user")
...
func TestMigrationColumnInfoReadsMySQLMetadataByPosition(t *testing.T) {
	...
	cfg := &conf.Configuration{Database: conf.Database{Prefix: prefix}}
	q := func(s string) string { return "`" + s + "`" }
	table := core.TableName(cfg, "metadata_probe")
	db.Exec("DROP TABLE IF EXISTS " + q(table))
	t.Cleanup(func() { db.Exec("DROP TABLE IF EXISTS " + q(table)) })
	...
	owner, ok, err := core.MigrationColumnInfo(db, table, "owner_id")
	require.NoError(t, err)
	require.True(t, ok)
	...
	flag, ok, err := core.MigrationColumnInfo(db, table, "flag")
	require.NoError(t, err)
	require.True(t, ok)
