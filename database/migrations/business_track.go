...
func RunBusinessMigrations(db *gorm.DB, config *conf.Configuration, list []business.Migration) (int, error) {
	...
	for _, migration := range list {
		tracked = append(tracked, core.TrackedMigration{Sequence: migration.Sequence, ID: migration.ID, Revision: migration.Revision, Up: migration.Up, VerifyBaseline: migration.VerifyBaseline, VerifySchema: migration.VerifySchema, VerifyUpgradeData: migration.VerifyUpgradeData})
	}
