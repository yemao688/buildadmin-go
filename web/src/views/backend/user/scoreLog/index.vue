...
const baTable = new baTableClass(
    new baTableApi(url),
    {
        column: [
            ...
            { label: t('user.scoreLog.Superior agent'), prop: 'admin_id', align: 'center', operator: '=', operatorPlaceholder: t('user.scoreLog.Superior agent'), comSearchRender: 'remoteSelect', remote: { pk: 'id', field: 'username', remoteUrl: '/admin/auth.Admin/index', params: { isTree: true } }, formatter: (row: anyObj, _column: any, cellValue: any, _index: number) => row.admin?.username || cellValue || '-', width: 130 },
