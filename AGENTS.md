...
- The development database has undergone the approved epoch reset; the old `go_migrations` ledger was rebuilt once. Official upstream identities remain immutable. Every migration remains prefix-safe (`mysql.prefix` is variable; never hard-code `ba_`). Destructive renames, type changes, and backfills must not rely on AutoMigrate.
