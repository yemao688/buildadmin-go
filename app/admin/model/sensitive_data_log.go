...
type SensitiveDataLog struct {
	...
	SensitiveData SensitiveData `gorm:"foreignKey:SensitiveID" json:"sensitive"`
}
...
func (s *SensitiveDataLogModel) GetOne(ctx *gin.Context, id int32) (sensitiveData SensitiveDataLog, err error) {
	err = s.sqlDB.Model(&SensitiveDataLog{}).
		Preload("Admin").
		Preload("SensitiveData").
		Joins("left join ba_admin admin on admin.id = ba_security_sensitive_data_log.admin_id").
		Joins("left join ba_security_sensitive_data sensitive_data on sensitive_data.id = ba_security_sensitive_data_log.sensitive_id").Where("ba_security_sensitive_data_log.id=?", id).First(&sensitiveData).Error
	return
}
...
func (s *SensitiveDataLogModel) List(ctx *gin.Context) (list []SensitiveDataLog, total int64, err error) {
	...
	db := s.sqlDB.Model(&SensitiveDataLog{}).
		Preload("Admin").
		Preload("SensitiveData").
		Joins("left join ba_admin admin on admin.id = ba_security_sensitive_data_log.admin_id").
		Joins("left join ba_security_sensitive_data sensitive_data on sensitive_data.id = ba_security_sensitive_data_log.sensitive_id").Where(whereS, whereP...)
