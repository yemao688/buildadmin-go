...
func TestMoneyLogScopeOwnerCentsBoundariesAndDelete(t *testing.T) {
	...
	spoof := &MoneyLog{AdminID: 40, UserID: u.ID, Money: 125, MoneyCents: true, Memo: "one"}
	...
	require.Error(t, f.money.Add(ctx, &MoneyLog{UserID: u.ID, Money: -1, MoneyCents: true, Memo: "too low"}))
	require.NoError(t, f.db.First(&got, u.ID).Error)
	require.Equal(t, int32(0), got.Money)
	require.NoError(t, f.db.Model(&User{}).Where("id = ?", u.ID).Update("money", math.MaxInt32-1).Error)
	require.Error(t, f.money.Add(ctx, &MoneyLog{UserID: u.ID, Money: 2, MoneyCents: true, Memo: "overflow"}))
	require.NoError(t, f.db.First(&got, u.ID).Error)
...
func TestMoneyLogConcurrentForUpdateFormsContinuousChain(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			defer wg.Done()
			errs <- f.money.Add(ctx, &MoneyLog{UserID: u.ID, Money: 100, MoneyCents: true, Memo: "chain"})
		}()
	}
	wg.Wait()
	close(errs)
...
func TestMoneyLogInsertFailureRollsBackBalanceAndDeleteCannotOrphan(t *testing.T) {
	...
	duplicate := MoneyLog{ID: 77, UserID: u.ID, AdminID: 20, Money: 1, Before: 0, After: 1, Memo: "existing"}
	require.NoError(t, f.db.Create(&duplicate).Error)
	err := f.money.Add(ctx, &MoneyLog{ID: 77, UserID: u.ID, Money: 100, MoneyCents: true, Memo: "duplicate"})
	require.Error(t, err)
	var got User
	require.NoError(t, f.db.First(&got, u.ID).Error)
	require.Equal(t, int32(0), got.Money)
