...
func TestGetFieldDefault(t *testing.T) {
	cases := []struct {
		...
		{
			name:  "switch INPUT 1",
			field: crudmodel.Field{Name: "status", DesignType: "switch", Default: "1", DefaultType: "INPUT"},
			want:  "status:'1'",
		},
		{
			name:  "switch INPUT 0 为无意义默认值",
			field: crudmodel.Field{Name: "status", DesignType: "switch", Default: "0", DefaultType: "INPUT"},
			want:  "",
		},
		{
			name:  "float NULL 不生成",
			field: crudmodel.Field{Name: "float", DesignType: "number", DefaultType: "NULL"},
			want:  "",
		},
		{
			name:  "weigh(映射为 number) NULL 不生成",
			field: crudmodel.Field{Name: "weigh", DesignType: "number", DefaultType: "NULL"},
			want:  "",
		},
		{
			name:  "number INPUT 非零输出原始数值",
			field: crudmodel.Field{Name: "weigh", DesignType: "number", Default: "5", DefaultType: "INPUT"},
			want:  "weigh:5",
		},
		{
			name:  "float INPUT 小数输出原始数值",
			field: crudmodel.Field{Name: "float", DesignType: "number", Default: "1.50", DefaultType: "INPUT"},
			want:  "float:1.5",
		},
		{
			name:  "number INPUT 0 为无意义默认值",
			field: crudmodel.Field{Name: "weigh", DesignType: "number", Default: "0", DefaultType: "INPUT"},
			want:  "",
		},
		{
			name:  "checkbox INPUT 多值拆数组",
			field: crudmodel.Field{Name: "checkbox", DesignType: "checkbox", Default: "opt0,opt1", DefaultType: "INPUT"},
			want:  "checkbox:['opt0', 'opt1']",
		},
		{
			name:  "string INPUT 引号包裹",
			field: crudmodel.Field{Name: "string", DesignType: "string", Default: "hello", DefaultType: "INPUT"},
			want:  "string:'hello'",
		},
		{
			name:  "string 含单引号改用双引号",
			field: crudmodel.Field{Name: "string", DesignType: "string", Default: "it's", DefaultType: "INPUT"},
			want:  `string:"it's"`,
		},
		{
			name:  "NONE 不生成",
			field: crudmodel.Field{Name: "string", DesignType: "string", DefaultType: "NONE"},
			want:  "",
		},
		{
			name:  "array 固定空数组",
			field: crudmodel.Field{Name: "arr", DesignType: "array"},
			want:  "arr: []",
		},
		{
			name:  "editor 无默认值输出空字符串",
			field: crudmodel.Field{Name: "content", DesignType: "editor"},
			want:  "content:''",
		},
		{
			name:  "remoteSelect INPUT 0 为无意义默认值",
			field: crudmodel.Field{Name: "ba_admin_id", DesignType: "remoteSelect", Default: "0", DefaultType: "INPUT"},
			want:  "",
		},
	}
...
func TestParseTableColumnsDefaultType(t *testing.T) {
	...
	byName := map[string]crudmodel.Field{}
