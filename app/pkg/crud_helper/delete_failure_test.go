...
func newDeleteFailureFixture(t *testing.T) (*gorm.DB, *conf.Configuration, deleteFailureFixture) {
	...
	modelDir := filepath.Join(utils.RootPath(), "app", "admin", "model", dirName)
	handlerDir := filepath.Join(utils.RootPath(), "app", "admin", "handler", dirName)
	...
	routerPath := filepath.Join(utils.RootPath(), "router", "registrar_set.go")
