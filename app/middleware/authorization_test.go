...
func newAuthorizationFixture(t *testing.T) *authorizationFixture {
	...
	require.NoError(t, db.AutoMigrate(&adminModel.AdminRule{}, &adminModel.AdminGroup{}, &adminModel.AdminGroupAccess{}))
	t.Cleanup(func() {
		sqlDB, err := db.DB()
		if err == nil {
			_ = sqlDB.Close()
	...
	return &authorizationFixture{
		auth: adminModel.NewAuthModel(db, nil, &conf.Configuration{}),
		db:   db,
	}
}

func (f *authorizationFixture) addRule(t *testing.T, name string) adminModel.AdminRule {
	t.Helper()
	rule := adminModel.AdminRule{Type: "button", Title: name, Name: name, Status: "1"}
...
func (f *authorizationFixture) grantRule(t *testing.T, uid int32, rules string) {
	t.Helper()
	group := adminModel.AdminGroup{
		Name:   "group-" + strconv.Itoa(int(uid)),
		Rules:  rules,
		Status: "1",
	}
	require.NoError(t, f.db.Create(&group).Error)
	require.NoError(t, f.db.Create(&adminModel.AdminGroupAccess{UID: uid, GroupID: group.ID}).Error)
}
