...
import (
	...
	"buildadmin-go/internal/pkg/crud_helper"
...
// 离线兜底对账。job 来源是 crud_specs/ 目录中所有 spec 的 dataScope.inheritFrom
// 声明反向聚合（声明式事实源，随仓库/镜像分发——重装/全新安装后 crud_log 为
// 空也能对账）。可选 [table] 参数限定只同步 inheritFrom 指向该主表的子表。
func newCascadeSyncCommand(cmdBootstrap CmdBootstrap) *cobra.Command {
	return &cobra.Command{
		Use:           "cascade:sync [table]",
		Short:         "按 crud_specs/ 中子表 inheritFrom 声明对账归属列（离线修复脏数据）",
...
// 同构：wire 注入 logger/config，自建 gorm 连接。specsDir 为级联声明事实源
// 目录（默认 crud_specs/，测试可注入临时目录）。
...
func NewCascadeHandler(logger *zap.Logger, config *conf.Configuration) *CascadeHandler {
	return &CascadeHandler{
		...
		config:   config,
		specsDir: crud_helper.DefaultSpecsDir(),
	}
}
...
// 由 crud_specs/ 中子表 spec 的 dataScope.inheritFrom 反向聚合而来。
type CascadeJob struct {
	ParentTable string     // 主实体表（admin_id 的事实源，PK=id）
	Children    []ChildRef // 声明 inheritFrom 指向主表的子表
}

// buildCascadeSyncJobsFromSpecs 扫描 specsDir 的 *.yaml 反向聚合级联同步任务。
// 决策说明：
//   - 事实源是 specs 目录（随仓库/镜像分发，重装不丢；registerOnly 登记表
//     与业务子表同样参与）——不依赖 crud_log 生成历史；
//   - 过滤规则：只有 dataScope.inheritFrom 非空且 table/byColumn 齐全的 spec
//     产出任务（该 spec 即子表，decl.ChildTable 是子表名，decl.ParentTable 是
//     主表名）；
//   - 聚合：同一主表的所有子表归入一个 CascadeJob（如 user ← user_money_log、
//     order_recharge 两个子表 → 一个 user job 含两个 children），按 spec 文件
//     名排序输出，确定性稳定；
//   - 任何标识符（表名/列名）不合法即报错，绝不进入 SQL。
func buildCascadeSyncJobsFromSpecs(specsDir string) ([]CascadeJob, error) {
	decls, err := crud_helper.ScanInheritDeclarations(specsDir)
	if err != nil {
		return nil, err
	}
	...
	for _, decl := range decls {
		child := ChildRef{ChildTable: decl.ChildTable, ByColumn: decl.ByColumn}
		if err := validateChildRef(decl.ParentTable, child); err != nil {
			return nil, err
		}
		idx, ok := byParent[decl.ParentTable]
		if !ok {
			idx = len(jobs)
			byParent[decl.ParentTable] = idx
			jobs = append(jobs, CascadeJob{ParentTable: decl.ParentTable})
		}
...
//
// 幂等：只动不一致行。表名 = 配置前缀 + spec 静态标识符，执行前对全部标识符
// （含拼接后的完整表名）做合法性校验，非法直接报错，绝不拼进 SQL。
// 执行前检查父表与子表存在性：spec 声明了 inheritFrom 但表尚未建（全新库
// 未 apply / 已 crud:delete 但 spec 残留）时，跳过该 job 并给出明确提示
// （fail-closed 的语义化处理，替代裸 1146）。
...
func runCascadeJob(db *gorm.DB, cfg *conf.Configuration, parentTable string, child ChildRef) (int64, error) {
	...
	exists, err := tableExistsByName(db, fullParent)
	if err != nil {
		return 0, err
	}
	if !exists {
		return 0, fmt.Errorf("cascade:sync: parent table %q missing: child %q declares inheritFrom targeting it (run crud:apply to materialize specs, or remove the child's inheritFrom declaration)", fullParent, fullChild)
	}
	exists, err = tableExistsByName(db, fullChild)
	if err != nil {
		return 0, err
	}
	if !exists {
		return 0, fmt.Errorf("cascade:sync: child table %q missing (spec declares inheritFrom but table was never materialized or was deleted); run crud:apply first", fullChild)
	}
	stmt := fmt.Sprintf(
...
// tableExistsByName 检查物理表是否存在（information_schema，前缀安全）。
func tableExistsByName(db *gorm.DB, table string) (bool, error) {
	var exists int64
	if err := db.Raw("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?", table).Scan(&exists).Error; err != nil {
		return false, fmt.Errorf("cascade:sync: check table %s existence: %w", table, err)
	}
	return exists > 0, nil
}

// Sync 执行级联对账主流程：
//   - 无参数：扫描 crud_specs/ 全部 inheritFrom 声明，按主表聚合后逐主表同步；
...
func (h *CascadeHandler) Sync(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 1 {
		...
		if len(filtered) == 0 {
			return fmt.Errorf("cascade:sync: no spec in crud_specs/ declares inheritFrom targeting table %q", target)
		}
		jobs = filtered
	}
