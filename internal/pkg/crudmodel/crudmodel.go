// Package crudmodel 承载 CRUD 生成器运行时契约类型：crud spec 解析、
// crud:apply 物化与 crud:generate 产物推导共用的数据结构（Table/Field/
// FormAttr/TableAttr/IndexSpec/ChangeField/CRUDFileManifest 等），以及
// crud_log 的 TEXT 列 JSON 载体类型（JSON_TABLE/JSON_FIELDS 及其
// Scan/Value 实现）。
//
// 这些类型与 internal/model 的持久化实体（Log/AdminRule 等）分离：
// 实体是贫血记录、驱动 schema 映射；契约类型是生成器的输入/输出形状，
// 只被 crud_helper 与 admin handler 消费，不参与数据库 schema。
// 下沉到 internal/pkg 层避免 internal/model 反向依赖生成器侧概念，
// 也便于生成器在无数据库连接的纯解析场景使用。
package crudmodel
