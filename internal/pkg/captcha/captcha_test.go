...
func TestDraw(t *testing.T) {
	filepath := filepath.Join(util.RootPath(), "public/static/images/captcha/click/bgs/1.png")
...
func TestDrawIcon(t *testing.T) {
	...
	iconImg, err := loadImage(util.RootPath() + "/public/static/images/captcha/click/icons/apple.png")
	if err != nil {
		fmt.Print(err)
	}

	iconBounds := iconImg.Bounds()
	newIcon := image.NewRGBA(iconImg.Bounds())

	for y := iconBounds.Min.Y; y < iconBounds.Max.Y; y++ {
		for x := iconBounds.Min.X; x < iconBounds.Max.X; x++ {

			// 获取原始像素的颜色
			oldColor := iconImg.At(x, y)

			// 解析颜色到 RGBA
			r, g, b, a := oldColor.RGBA()
			if r > 0 && g > 0 && b > 0 && a > 0 {
				// 创建新的颜色，并设置到新图像中
				newColor := color.RGBA{128, 128, 128, 128}
				newIcon.Set(x, y, newColor)
			}
...
func TestWriteCurve(t *testing.T) {
	...
	// 设置背景
	backgroundColor := color.RGBA{uint8(243), uint8(251), uint8(254), 255}
