...
func TestBuildFileManifestForFieldsContainsExistingRelationProvider(t *testing.T) {
	dir := filepath.Join(utils.RootPath(), "internal", "admin", "model", "relation_manifest_test")
	...
	manifest, err := BuildFileManifestForFields(crudmodel.Table{Name: "orders"}, []crudmodel.Field{{Form: crudmodel.FormAttr{RemoteTable: "owner", RemoteModel: "internal/admin/model/relation_manifest_test/Owner.go", RelationFields: "name"}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildFileManifestForFieldsAlwaysClassifiesRelationProviderAsShared(t *testing.T) {
	dir := filepath.Join(utils.RootPath(), "internal", "admin", "model", "relation_manifest_absent_test")
	t.Cleanup(func() { _ = os.RemoveAll(dir) })
	manifest, err := BuildFileManifestForFields(crudmodel.Table{Name: "orders"}, []crudmodel.Field{{Form: crudmodel.FormAttr{RemoteTable: "owner", RemoteModel: "internal/admin/model/relation_manifest_absent_test/Owner.go", RelationFields: "name"}}})
	if err != nil {
		t.Fatal(err)
	}
	provider := filepath.Join(dir, "provider.go")
