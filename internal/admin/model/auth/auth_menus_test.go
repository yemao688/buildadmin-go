...
func TestGetMenusUsesAdministratorRules(t *testing.T) {
	...
	if err := db.AutoMigrate(&AdminRule{}, &AdminGroup{}, &AdminGroupAccess{}); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Create([]AdminGroupAccess{{UID: 1, GroupID: 1}, {UID: 2, GroupID: 2}}).Error; err != nil {
		t.Fatal(err)
	}

	auth := NewAuthModel(db, nil, &conf.Configuration{})
