...
func TestUserCascadeOwnersRegistration(t *testing.T) {
	...
	require.Len(t, owners, 1, "CascadeOwners must register user_money_log")
	owner := owners[0]
	require.Equal(t, "user_money_log", owner.Table)
...
func TestUserSyncCascadeOwnersMySQL(t *testing.T) {
	...
	require.NoError(t, db.Exec("CREATE TABLE `"+prefix+"user_money_log` (`id` int NOT NULL AUTO_INCREMENT, `user_id` int NOT NULL, `admin_id` int NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4").Error)
	t.Cleanup(func() { _ = db.Exec("DROP TABLE IF EXISTS `" + prefix + "user_money_log`").Error })
