...
func closureRows(t *testing.T, db *gorm.DB) []model.AdminClosure {
	t.Helper()
	var rows []model.AdminClosure
	if err := db.Model(&model.AdminClosure{}).Order("ancestor_id, descendant_id").Find(&rows).Error; err != nil {
		t.Fatalf("list closure rows: %v", err)
	}
	return rows
}

func ancestorIDs(rows []model.AdminClosure, descendantID int32) []int32 {
	var ids []int32
	for _, r := range rows {
...
func adminParentID(t *testing.T, db *gorm.DB, id int32) *int32 {
	t.Helper()
	var parent *int32
	if err := db.Model(&model.Admin{}).Where("id = ?", id).Select("parent_id").Scan(&parent).Error; err != nil {
...
func TestAdminHierarchyLinkNewNode(t *testing.T) {
	...
	_ = db.Migrator().DropTable(&model.AdminClosure{}, &model.Admin{})
	if err := db.AutoMigrate(&model.Admin{}, &model.AdminClosure{}); err != nil {
...
func TestAdminHierarchyLinkNewNodeOrphanParent(t *testing.T) {
	...
	if !errors.Is(err, adminmodel.ErrHierarchyOrphanParent) {
		t.Fatalf("expected adminmodel.ErrHierarchyOrphanParent, got %v", err)
	}
...
func TestAdminHierarchyMoveRejectsInvalid(t *testing.T) {
	...
	}); !errors.Is(err, adminmodel.ErrHierarchyOrphanParent) {
...
func TestAdminHierarchyMoveRejectsInconsistentParent(t *testing.T) {
	...
	if err := db.Model(&model.Admin{}).Where("id = ?", b.ID).Update("parent_id", a.ID+1000).Error; err != nil {
		t.Fatalf("corrupt parent_id: %v", err)
	}

	if err := db.Transaction(func(tx *gorm.DB) error {
		return h.MoveSubtree(context.Background(), tx, b.ID, nil)
	}); !errors.Is(err, adminmodel.ErrHierarchyIntegrity) {
		t.Fatalf("expected adminmodel.ErrHierarchyIntegrity, got %v", err)
	}
}
...
func TestAdminHierarchyModelAddRollbackOnClosureFailure(t *testing.T) {
	...
	var count int64
	if err := db.Model(&model.Admin{}).Where("username = ?", "rollback").Count(&count).Error; err != nil {
...
func TestAdminHierarchyAnchorRowBusyFailsClosed(t *testing.T) {
	...
	if !errors.Is(err, adminmodel.ErrHierarchyIntegrity) {
		t.Fatalf("expected adminmodel.ErrHierarchyIntegrity, got %v", err)
	}
}
...
func TestAdminHierarchyClosureOnlyRealAdmins(t *testing.T) {
	...
	for _, r := range rows {
		for _, id := range []int32{r.AncestorID, r.DescendantID} {
			var exists int64
			if err := db.Model(&model.Admin{}).Where("id = ?", id).Count(&exists).Error; err != nil {
	...
	if err := db.Model(&model.AdminClosure{}).Where("depth = ?", 0).Count(&depthZeroCount).Error; err != nil {
...
func countGroupAccessRows(t *testing.T, db *gorm.DB) int64 {
	t.Helper()
	var count int64
	if err := db.Model(&model.AdminGroupAccess{}).Count(&count).Error; err != nil {
