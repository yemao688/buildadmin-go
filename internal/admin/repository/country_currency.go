...
func (s *CountryCurrencyRepository) readScopedDB(ctx *gin.Context, db *gorm.DB) *gorm.DB {
	if s.Policy.Mode == data_scope.ModeNone {
		return db
	}
	if s.Enforcer == nil {
...
func (s *CountryCurrencyRepository) GetOne(ctx *gin.Context, id int64) (countryCurrency model.CountryCurrency, err error) {
...
func (s *CountryCurrencyRepository) Add(ctx *gin.Context, countryCurrency model.CountryCurrency) error {
	if s.Policy.Mode != data_scope.ModeNone {
		if s.Enforcer == nil {
...
func (s *CountryCurrencyRepository) Edit(ctx *gin.Context, countryCurrency model.CountryCurrency) error {
	if s.Policy.Mode != data_scope.ModeNone {
		if s.Enforcer == nil {
			return data_scope.ErrScopedAccessDenied
		}
	...
	return s.Transaction(ctx, func(tx *gorm.DB) error {
		...
		case 0:
			var visible int64
			if err := tx.Table(s.TableName).Model(&model.CountryCurrency{}).Where("id = ?", countryCurrency.ID).Count(&visible).Error; err != nil {
				return err
			}
			if visible == 1 {
				return nil
...
func (s *CountryCurrencyRepository) Del(ctx *gin.Context, ids interface{}) error {
	...
	return s.Transaction(ctx, func(tx *gorm.DB) error {
		...
		var visible int64
		if err := tx.Table(s.TableName).Model(&model.CountryCurrency{}).Where("id IN ?", normalizedIDs).Count(&visible).Error; err != nil {
			return err
		}
		...
		res := tx.Table(s.TableName).Where("id IN ?", normalizedIDs).Delete(&model.CountryCurrency{})
		if err := res.Error; err != nil {
			return err
		}
