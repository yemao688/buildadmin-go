...
func TestMoneyLogScopeOwnerBoundariesAndDelete(t *testing.T) {
	...
	require.Error(t, f.money.Add(ctx, &model.MoneyLog{UserID: u.ID, Money: -1, Memo: "too low"}))
	require.NoError(t, f.db.First(&got, u.ID).Error)
...
func TestMoneyLogConcurrentForUpdateFormsContinuousChain(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			defer wg.Done()
			errs <- f.money.Add(ctx, &model.MoneyLog{UserID: u.ID, Money: 1.00, Memo: "chain"})
		}()
	}
	wg.Wait()
	close(errs)
...
func TestMoneyLogInsertFailureRollsBackBalanceAndDeleteCannotOrphan(t *testing.T) {
	...
	err := f.money.Add(ctx, &model.MoneyLog{ID: 77, UserID: u.ID, Money: 1.00, Memo: "duplicate"})
	require.Error(t, err)
	var got model.User
	require.NoError(t, f.db.First(&got, u.ID).Error)
	require.Equal(t, float64(0), got.Money)
