...
import (
	...
	"errors"
	"fmt"
	routinedto "buildadmin-go/internal/admin/dto/routine"
...
func (h *ConfigHandler) SendTestMail(ctx *gin.Context) {
	...
	if err := dialer.DialAndSend(message); err != nil {
		adminhandler.JsonReturn(ctx, http.StatusOK, 0, "Mail sending service unavailable", err.Error())
		return
	}
	adminhandler.Success(ctx, "test mail sent successfully~")
}
