...
func wireApp(configuration *conf.Configuration, lumberjackLogger *lumberjack.Logger, zapLogger *zap.Logger) (*App, func(), error) {
	...
	groupHandler := user2.NewGroupHandlerWithAuth(zapLogger, groupModel, adminRuleModel, authModel, modelAuthModel)
	groupRegistrar := user2.NewGroupRegistrar(groupHandler)
	ruleModel := user.NewRuleModel(gormDB, configuration)
	ruleHandler := user2.NewRuleHandlerWithAuth(zapLogger, ruleModel, authModel, modelAuthModel)
